...
Report status: **LOCAL RELEASE COMPLETE; PUBLIC SOURCE REPOSITORY, COLLABORATION SURFACE, AND VERCEL WEBSITE VERIFIED**
...
| Field | Value |
| --- | --- |
| Repository root | The checkout root containing this report |
...
| Public source repository | **Authorized and verified public** | `https://github.com/Zen-ctrl/morph`, default branch `main`, visibility `PUBLIC`. |
...
| Publication boundary | **Public source and website authorized** | npm publication, a GitHub release, paid evaluation, and broader hosted capability remain separate actions. |
...
- Architecture-editorial light and dark visual themes, animated MORPH wordmarks, project
  graphics, responsive layouts, and reduced-motion behavior. Theme choice remains
...
| White paper verification | `0` | The HTML paper rendered locally. The PDF contains 23 nonblank pages, is 326,114 bytes, has no extracted private operational identifiers, and is byte-identical to the website copy. Pages 1, 11, and 23 passed visual inspection. |
| Contributor white paper build smoke | `0` | The documented Python generator compiled, produced canonical and website-copy PDFs, and verified byte-identical SHA-256 `ba0d938da00dedbec8343ea0d1b8cd6fbe29d5e071c813ded053a70ad8ed153f`. |
...
to prefer the cache while permitting dependency metadata retrieval, then kept every
installed MORPH runtime check offline. The corrected private-history workflow passed.
...
source commit: unavailable after pre-publication source-history sanitization
...
13. Public source visibility is authorized. No owner license, npm publication, or GitHub
    release has been selected. Public visibility alone does not grant reuse rights.
...
| Vercel internal identifiers | Intentionally omitted from the public repository |
...
## Website refresh, 2026-09-25

The public workbench and white paper now use an architecture-editorial visual system:
compact mastheads, stacked index navigation, warm paper colors, ruled content areas,
and four original, architecture-inspired feature images. The homepage also has a new
isometric structure illustration and matching favicon. The compiler and evaluation
logic were not changed.

Local verification for this refresh was deliberately narrow: the workbench production
build passed, `App.smoke.test.tsx` passed 1 of 1 tests, and Biome checked the six
edited source files. The homepage and white paper were visually inspected in the
local browser. No model-quality evaluation or benchmark was rerun for this design
change.

The checkout was linked to the existing Vercel project `morph`. The production deployment
reached `READY`, was aliased to `https://morph-one-jade.vercel.app`, and the live homepage
was visually checked. Internal hosting account, project, and deployment identifiers are
