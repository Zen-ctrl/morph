...
describe("workbench UI smoke", () => {
  it("binds worker results, exports an importable artifact, and rejects stale canceled output", async () => {
    ...
    expect(document.querySelector(".selection-metric span")?.textContent).toBe("prompt tokens");
    ...
    for (const label of ["Model text", "Saved result", "Decision details", "Rebuilt JSON"]) {
      await act(async () => {
    ...
    expect(document.querySelector(".sr-status")?.textContent).toContain(
      "Saved result verified and opened",
    );
